...
mixin(joinFnBinds((){
	FnBind[] ret = [
		...
		{q{int}, q{TTF_GetFontKerningSize}, q{TTF_Font* font, int prevIndex, int index}}, //NOTE: deprecated
	];
	if(sdlTTFSupport >= SDLTTFSupport.v2_0_14){
		FnBind[] add = [
			{q{int}, q{TTF_GetFontKerningSizeGlyphs}, q{TTF_Font* font, ushort previousCh, ushort ch}},
		];
		ret ~= add;
	}
	if(sdlTTFSupport >= SDLTTFSupport.v2_0_18){
		FnBind[] add = [
			...
			{q{int}, q{TTF_GetFontKerningSizeGlyphs32}, q{TTF_Font* font, uint previousCh, uint ch}},
			{q{int}, q{TTF_SetDirection}, q{int direction}}, //NOTE: deprecated
			{q{int}, q{TTF_SetScript}, q{int script}}, //NOTE: deprecated
			{q{int}, q{TTF_MeasureText}, q{TTF_Font* font, const(char)* text, int measureWidth, int* extent, int* count}},
			{q{int}, q{TTF_MeasureUTF8}, q{TTF_Font* font, const(char)* text, int measureWidth, int* extent, int* count}},
			{q{int}, q{TTF_MeasureUNICODE}, q{TTF_Font* font, const(ushort)* text, int measureWidth, int* extent, int* count}},
	...
	if(sdlTTFSupport >= SDLTTFSupport.v2_20){
		FnBind[] add = [
