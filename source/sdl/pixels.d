...
enum SDL_ALPHA_OPAQUE_FLOAT = 1f;

enum SDL_ALPHA_TRANSPARENT = 0;

enum SDL_ALPHA_TRANSPARENT_FLOAT = 0f;
...
pragma(inline,true) extern(C) nothrow @nogc pure @safe{
	...
	ubyte SDL_PIXELFLAG(uint x)   => cast(ubyte)((x >> 28) & 0x0F);
	ubyte SDL_PIXELTYPE(uint x)   => cast(ubyte)((x >> 24) & 0x0F);
	ubyte SDL_PIXELORDER(uint x)  => cast(ubyte)((x >> 20) & 0x0F);
	ubyte SDL_PIXELLAYOUT(uint x) => cast(ubyte)((x >> 16) & 0x0F);
	
	ubyte SDL_BITSPERPIXEL(uint x) =>
		SDL_ISPIXELFORMAT_FOURCC(x) ? 0 : cast(ubyte)((x >> 8) & 0xFF);
	
	ubyte SDL_BYTESPERPIXEL(uint x) =>
...
pragma(inline,true) extern(C) nothrow @nogc pure @safe{
	...
	bool SDL_ISCOLOURSPACE_MATRIX_BT601(SDL_Colourspace x) =>
		SDL_COLOURSPACEMATRIX(x) == SDL_MatrixCoefficients.bt601 ||
		SDL_COLOURSPACEMATRIX(x) == SDL_MatrixCoefficients.bt470BG;
	
	bool SDL_ISCOLOURSPACE_MATRIX_BT709(SDL_Colourspace x) =>
		SDL_COLOURSPACEMATRIX(x) == SDL_MatrixCoefficients.bt709;
	
	bool SDL_ISCOLOURSPACE_MATRIX_BT2020_NCL(SDL_Colourspace x) =>
		SDL_COLOURSPACEMATRIX(x) == SDL_MatrixCoefficients.bt2020NCL;
	
	bool SDL_ISCOLOURSPACE_LIMITED_RANGE(SDL_Colourspace x) =>
		SDL_COLOURSPACERANGE(x) != SDL_ColourRange.full;
	
	bool SDL_ISCOLOURSPACE_FULL_RANGE(SDL_Colourspace x) =>
		SDL_COLOURSPACERANGE(x) == SDL_ColourRange.full;
