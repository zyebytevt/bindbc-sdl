...
extern(C) nothrow{
	...
	//These aren't in SDL_mixer.h and are just here as a convenient and visible means to add the proper attributes to these callbacks.
...
mixin(joinFnBinds((){
	FnBind[] ret = [
		...
		{q{void}, q{Mix_SetPostMix}, q{callbackVPVPUbI mixFunc, void* arg}},
		{q{void}, q{Mix_HookMusic}, q{callbackVPVPUbI mixFunc, void* arg}},
		{q{void}, q{Mix_HookMusicFinished}, q{callbackV musicFinished}},
		{q{void*}, q{Mix_GetMusicHookData}, q{}},
		{q{void}, q{Mix_ChannelFinished}, q{callbackVI channelFinished}},
	...
	if(sdlMixerSupport >= SDLMixerSupport.v2_0_2){
		FnBind[] add = [
			{q{int}, q{Mix_OpenAudioDevice}, q{int frequency, ushort format, int channels, int chunkSize, const(char)* device, int allowedChanges}},
			{q{SDL_bool}, q{Mix_HasChunkDecoder}, q{const(char)* name}},
			//[q{SDL_bool}, q{Mix_HasMusicDecoder}, q{const(char)*}], //Declared in SDL_mixer.h, but not implemented
		];
		ret ~= add;
	}
	if(sdlMixerSupport >= SDLMixerSupport.v2_6){
		FnBind[] add = [
